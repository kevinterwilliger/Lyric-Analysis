...
```{r include=F}
current_data = read.csv("data_Oct1st.csv",header=T) %>% select(-c(1,2,3,4)) %>% filter(Loudness != 9999)
```

### Question 1. Validation and Training Sets {#q1}

```{r}
train = current_data %>% select(2,c(8:18)) %>% sample_frac(0.7) 
test = current_data %>% select(2,c(8:18)) %>% setdiff(train)
```
...
```{r}
predict(ridge_cv.out, type = "coefficients", s = bestlam)
```

```{r}
x_train = model.matrix(Rank~., train)[,-1] 
x_test = model.matrix(Rank~., test)[,-1]
y_train = train$Rank
y_test = test$Rank
...
```{r}
ggplot() +
  geom_point(aes(x = test$Rank, y = ridge_pred),
             colour = 'blue') +
  geom_line(aes(x = test$Rank, y = test$Rank),
...
ggplot() +
  geom_point(aes(x = test$Rank, y = lasso_pred),
             colour = 'blue') +
  geom_line(aes(x = test$Rank, y = test$Rank),
            colour = 'red') + # plotting Y againts itself produces a red 45 degree line 
  ggtitle('ridge predictions vs salary') +
